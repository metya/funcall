class ChainableObject(object):
    ...
    def __rshift__(self, other, *args, **kwargs):
        if other.__name__ == 'unpack':
            return self.__func(self.__value)
        elif other.__name__ == 'print':
            return print(self.__func(self.__value))
        
        return ChainableObject(
            self,
            lambda _: other(self.__func(self.__value))
        )
...
def chainable(x):
    '''chainable(x) -> chainable object with >> operator

    Convert an object to a chainable object.

    Example
    -------
    >>> chainable(10) >> range >> filter(lambda x: x > 2) >> map(float) >> puts #TODO: fix that 
    [3.0, 4.0, 5.0, 6.0, 7.0, 8.0, 9.0]
    >>> chainable(10) >> range >> sum >> float
    45.0
...
def unpack(x):
    if not isinstance(x, ChainableObject):
        raise TypeError('Hey, baby, it\'s specially for ChainableObjects ;)')
    return x >> unpack
